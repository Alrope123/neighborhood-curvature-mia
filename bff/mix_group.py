...
def main(args):
    ...
    print([good_base_member_groups, good_pollution_member_groups, good_base_nonmember_groups, good_pollution_nonmember_groups])
    assert False
    ...
    for i, group in enumerate(good_base_member_groups):
        ...
        assert len(documents) > member_base_size
        ...
        assert len(documents) > nonmember_pollution_size
        np.random.shuffle(documents)
        final_documents.extends(documents[:nonmember_pollution_size])
        final_membership.extends([False] * nonmember_pollution_size)
    ...
    for i, group in enumerate(good_base_nonmember_groups):
        ...
        documents = membership_info[group]['documents']
        assert len(documents) > nonmember_base_size
        np.random.shuffle(documents)
        final_documents = documents[:nonmember_base_size]
        final_membership = [False] * nonmember_base_size

        documents = membership_info[other_group]['documents']
        assert len(documents) > member_pollution_size
        np.random.shuffle(documents)
        final_documents.extends(documents[:member_pollution_size])
        final_membership.extends([True] * member_pollution_size)
