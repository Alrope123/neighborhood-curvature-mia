...
if __name__ == '__main__':
    ...
    for top_k in [1, 3, 5, 10, 30, 50]:
        ...
        for group, predictions in group_results_members.items():
            cur_member_predictions.append(np.mean(sorted(predictions, reversed=True)[top_k]))
        for group, predictions in group_results_nonmembers.items():
            cur_nonmember_predictions.append(np.mean(sorted(predictions, reversed=True)[top_k]))
