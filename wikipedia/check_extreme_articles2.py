...
def main(args):
    ...
    nonmember_dict_path = "/gscratch/h2lab/alrope/neighborhood-curvature-mia/wikipedia/out/pile_nonmember_text.pkl"
    nonmember_dict = {}
    if nonmember_dict == {}:
        with open(nonmember_dict_path, 'rb') as f:
            nonmember_dict = pkl.load(f)
            print("Loaded in {} wikipedia titled matched.".format(len(nonmember_dict)))
    ...
    for i, (data_path, filename) in enumerate(tqdm(iterate_files(data_dir))):
        ...
        for j, dp in enumerate(custom_open_yield(data_path, suffix=".jsonl")):
            ...
            if title in nonmember_dict and qualified(score):
                perfect_set.add(title)

    print("Size of the resulting set: {}".format(len(perfect_set)))

    # Save the membership info
    with open(os.path.join(save_dir, "worst_set.pkl"), "wb") as f:
